...
// Design a linear-time certification algorithm to check whether an array pq[] is a max-oriented heap.
...
fn checkIfMaxPQ<T: Copy + PartialOrd + std::fmt::Debug>(array: &Vec<Option<T>>) -> bool {
    match array[0] {
        None => return false,
        _ => {}
    }
    ...
    // No need to check the nodes at the bottom of the tree
    while index <= array.len() / 2 {
        if array[index] < array[2 * index] 
            || array[index] < array[(2 * index) + 1] 
            || 2 * index + 1 >= array.len() // Check against out of bounds error
            {return false}
        index += 1;
    }
