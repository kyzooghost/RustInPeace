...
// Implement eager delete() for DoubleProbingSeparateChainingHashST and DoubleHashingLinearProbingHashST

// Lol...needing to rehash the entire table for DoubleHashingLinearProbingHashST after every deletion
// whereas eager deletion in LinearProbingHashST only requires re-hashing the elements in the probe sequence
// following the deleted element. 

// Seems like no point doing eager deletion for double hashing linear probe hash ST, because the efficiency loss
// is much larger compared to lazy deletion
