...
// for each key-value pair that is set to the number of entries in the table at the time that pair is inserted. 

// Then implement a method that deletes all keys (and associated values) for which the field is greater than a given integer k. Note : This extra functionality is use- ful in implementing the symbol table for a compiler.

// Add usize field to STNode = number of entries in table at the time the pair is inserted
// Add method to delete all keys where field greater than a given integer k
