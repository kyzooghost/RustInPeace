...
// Delete method that set value to null (but not remove key)
// Later remove pair in resize()
// When to call resize() - # of tombstone items, and # of empty positions
// Call resize() if tomestone: size ratio is >1:4
...
, U: Copy + Clone + PartialOrd + PartialEq + std::fmt::Debug> 
    ...
    pub fn put(&mut self, key: T, val: U) {
        if self.num_of_keys >= self.size / 2 || self.tombstone_count * 4 > self.num_of_keys {
            // println!("resized in put");
            // println!("4*tombstone_count = {:?}", self.tombstone_count * 4);
            // println!("num_of_keys = {:?}", self.num_of_keys);
