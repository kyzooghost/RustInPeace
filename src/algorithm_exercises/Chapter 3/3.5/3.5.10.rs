...
impl<T: Clone + PartialOrd + PartialEq + Copy + std::fmt::Debug, U: Clone + PartialOrd + PartialEq + Copy> RedBlackBST<T, U> {
    ...
    fn _is23(&self, pointer: *mut Node<T, U>) -> bool {
    ...
    fn _put(&mut self, mut node_pointer: *mut Node<T, U>, key: T, value: U) -> *mut Node<T, U> {
        ...
        unsafe {
            ...
            // REMOVE THIS FROM DUPLICATE KEY DATA STRUCTURE - DO NOT JUST STOP AT SEARCH HIT AND REPLACE VALUE
            // else {
        ...
        unsafe {
            ...
            // Else if >= current key
                ...
                if key == (*pointer).key {
                    ...
                    // MODIFICATION FOR DUPLICATE KEYS - RECURSE INTO POINTER ITSELF, START SEARCHING DUPLICATE TO DELETE FRMO THERE
                    pointer = self._delete(pointer, key);
                } else {
