...
pub struct HashMultiSet<T: Clone + PartialOrd + PartialEq + Copy + std::fmt::Debug + Hash, U: Clone + PartialOrd + PartialEq + Copy + Hash> {
    st: LinearProbingHashTable<T,U>
}

impl<T: Clone + PartialOrd + PartialEq + Copy + std::fmt::Debug + Hash, U: Clone + PartialOrd + PartialEq + Copy + std::fmt::Debug + Hash> HashMultiSet<T, U> {
    pub fn new() -> Self {
        // Choose 997 as default size
        let st = LinearProbingHashTable::new(997);
        HashMultiSet {st: st}
    }
