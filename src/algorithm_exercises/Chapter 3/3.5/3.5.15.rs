...
fn run() -> Result<(), Box<dyn Error>> {
    ...
    let inverted_st = invert(st);

    println!("{:?}", inverted_st.keys());
    println!("{:?}", inverted_st.keys().len());
    println!("{:?}", inverted_st.get(&"TTT".to_string()));

    Ok(())
}

fn invert<T: Clone + PartialEq + std::fmt::Debug + Hash, U: Clone + PartialEq + std::fmt::Debug + Hash>(st: LinearProbingHashTable<T, U>) -> LinearProbingHashTable<U, T> {
    let mut inverted_st: LinearProbingHashTable<U, T> = LinearProbingHashTable::new(2 * st.size());
    let keys = st.keys();

    for key in keys {
        let values = st.getAll(key);

        for value in values {
            if !inverted_st.contains(value) {
                inverted_st.put(value.clone(), key.clone());
            }
        }
    }
