...
// 2-3 trees without balance restriction. 
// Develop an implementation of the basic symbol-table API 
// that uses 2-3 trees that are not necessarily balanced as the underlying data structure. 
// Allow 3-nodes to lean either way. 
// Hook the new node onto the bottom with a black link when inserting into a 3-node at the bottom. 
// Run experiments to develop a hypothesis estimating the average path length in a tree built from N random insertions.

// Allow 3-nodes to lean either way. 
// Hook the new node onto the bottom with a black link when inserting into a 3-node at the bottom

// No need to rotate left is right-leaning red link
// Insert as red, check if parent red
// If insert into existing black link, will be red-link
// Delete, deleteMin and deleteMax implementations unchanged
...
impl<T: Clone + PartialOrd + PartialEq + Copy + std::fmt::Debug, U: Clone + PartialOrd + PartialEq + Copy> BinarySearchTree<T, U> {
    ...
    fn _put(&mut self, node_pointer: *mut Node<T, U>, key: T, value: U) -> *mut Node<T, U> {
        ...
        unsafe {
            ...
            // Cannot allow two consecutive red links
            if self.isRed(node_pointer) && self.isRed((*node_pointer).left_child) && !(*node_pointer).left_child.is_null() {
                (*(*node_pointer).left_child).colour = Colour::Black;
            }

            if self.isRed(node_pointer) && self.isRed((*node_pointer).right_child) && !(*node_pointer).right_child.is_null() {
                (*(*node_pointer).right_child).colour = Colour::Black;
            }

            // If left and right child red link, then flip colour
