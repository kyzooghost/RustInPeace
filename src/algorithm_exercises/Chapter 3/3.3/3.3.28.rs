...
// Develop an implementation of the basic symbol-table API that uses 
// balanced 2-3-4 trees as the underlying data structure, 
// using the red-black representation and a bottom-up insertion method 
// based on the same recursive approach as Algorithm 3.4. 
// Your insertion method should split only the sequence of 4-nodes (if any) on the bottom of the search path.

// So split on the bottom only, not on the way down
...
impl<T: Clone + PartialOrd + PartialEq + Copy + std::fmt::Debug, U: Clone + PartialOrd + PartialEq + Copy> BinarySearchTree<T, U> {
    ...
    fn _put(&mut self, node_pointer: *mut Node<T, U>, key: T, value: U) -> *mut Node<T, U> {
        ...
        unsafe {
            ...
            // On way up here, check if one of children is new node and is four-node
            if self.isRed((*node_pointer).left_child) &&
                self.isRed((*node_pointer).right_child) &&
                self._subtree_size((*node_pointer).left_child) == 1 &&
                self._subtree_size((*node_pointer).right_child) == 1 
            {
