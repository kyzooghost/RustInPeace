...
// 1.3.38 GeneralizedQueue

struct GeneralizedQueue_List<T> where T: std::fmt::Debug {
    size: usize,
    list: List<T>
}

struct GeneralizedQueue_Vector<T> where T: std::fmt::Debug {
    size: usize,
    vec: Vec<T>
}


impl<T> GeneralizedQueue_Vector<T> where T: std::fmt::Debug {
    pub fn isEmpty(&self) -> bool {
        self.size == 0
    }

    pub fn new() -> Self {
        GeneralizedQueue_Vector {size: 0, vec: Vec::new()}
    }
    ...
    pub fn delete(&mut self, k: usize) -> Option<T> {
        if self.size == 0 {None}
        else if k >= self.size {None}
        else {
            self.size = self.size - 1;
            Some(self.vec.remove(k))
        }
    }
}

impl<T> GeneralizedQueue_List<T> where T: std::fmt::Debug {
    pub fn isEmpty(&self) -> bool {
        self.size == 0
    }

    pub fn new() -> Self {
        GeneralizedQueue_List {size: 0, list: List::new()}
    }

    pub fn insert(&mut self, elem: T) {
        self.list.insert_at_head(elem);
        self.size = self.size + 1;
    }
    ...
    pub fn delete(&mut self, k: usize) -> Option<T> {
        if self.size == 0 {None}
            ...
            self.size = self.size - 1;
            self.list.remove_at_index(self.size - k)
        }
    }
}

fn main () {
    let mut queue = GeneralizedQueue_Vector::new();
    queue.insert(1);
    println!("{:?}", queue.delete(0));
}
