...
// Threading. 
// Your goal is to support an extended API ThreadedST that supports the following additional operations in constant time:
...
// and modify put(), deleteMin(), deleteMax(), and delete() to maintain these fields.
...
pub struct Node<T: Clone + PartialOrd + PartialEq + Copy + std::fmt::Debug, U: Clone + PartialOrd + PartialEq + Copy> {
    ...
    predecessor: *mut Node<T, U>,
    successor: *mut Node<T, U>,
}
...
impl<T: Clone + PartialOrd + PartialEq + Copy + std::fmt::Debug, U: Clone + PartialOrd + PartialEq + Copy> BinarySearchTree<T, U> {
    ...
    pub fn findPredecessor(&self, pointer: *mut Node<T, U>) -> *mut Node<T, U> {
        unsafe {
            if self.is_empty() || self.min().unwrap() == (*pointer).key || (*pointer).left_child.is_null() { return ptr::null_mut() }
            self._max((*pointer).left_child)
        }
    }

    pub fn findSuccessor(&self, pointer: *mut Node<T, U>) -> *mut Node<T, U> {
        unsafe {
            if self.is_empty() || self.max().unwrap() == (*pointer).key || (*pointer).right_child.is_null() { return ptr::null_mut() }
            self._min((*pointer).right_child)
        }
    }
    ...
    fn _put(&mut self, node_pointer: *mut Node<T, U>, key: T, value: U) -> *mut Node<T, U> {
        ...
        unsafe {
            ...
            (*node_pointer).predecessor = self.findPredecessor(node_pointer);
            (*node_pointer).successor = self.findSuccessor(node_pointer);
        }
    ...
    fn _deleteMax(&self, node_pointer: *mut Node<T, U>) -> *mut Node<T, U> {
        unsafe {
            ...
            // Base case, can no longer recurse into right child
    ...
    pub fn next(&self, key: T) -> Option<T> {
        ...
        // Recursive search for key, start from root
        self._next(self.root, key)
    }

    // Return value associated with key in the subtree rooted at node_pointer
    // If key not present, return None
    fn _next(&self, node_pointer: *mut Node<T, U>, key: T) -> Option<T> {
        if self.is_empty() {return None}
        ...
        unsafe {
            // If given key < root key, recurse into left child
            if key < (*node_pointer).key {return self._next((*node_pointer).left_child, key)}
            // If given key > root key, recurse into right child
            else if key > (*node_pointer).key {return self._next((*node_pointer).right_child, key)}
            // Else given key == root key, search hit
            else {
                if (*node_pointer).successor.is_null() {return None}
                else {
                    return Some( (*(*node_pointer).successor).key )
                }
            }
        }
    }
